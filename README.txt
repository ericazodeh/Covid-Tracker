NOTE: This program was ran using an AWS remote database that is no longer running. If you'd like to
use this repo locally or remotely, you can simply provide your host/password credentials and create a MySQL 
schema called 'newSchema', and a table called "data". It should work after that!

1. git clone this repo to a folder of your choice
In the terminal:
2. run 'cd to/your/file/path'
3. set up a Python virtual environment (if you would like)
	a. Run 'python -m venv env' 
	b. Run 'activate'
4. install all required modules by running pip install -r requirements.txt (these should
   be all of them but if a ModuleNotFound error comes, just run 'pip install <moduleName>'
5. Run program with 'python covid.py'